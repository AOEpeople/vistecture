...
visRenderer.RenderNetwork = function (
  container,
  projectData,
  configurationData
) {
  ...
  for (let missingAppId in projectData.missingApplications) {
    ...
    node = {
      id: missingApp.id,
      font: { color: "#ffffff" },
      title: "MISSING! " + missingApp.title,
      label: "MISSING!" + missingApp.name,
      color: {
        border: "#ff0000",
        background: "#ee0000",
        highlight: { background: "#ff0000", border: "#ffaaaa" },
      },
    };
    nodes.push(node);
  }
  ...
  for (let unincludedAppId in projectData.unincludedApplications) {
    ...
    node = {
      ...
      color: {
        border: "#ee0000",
        background: "#666666",
        highlight: { background: "#aaaaaa", border: "#bbaaaa" },
      },
    };
    nodes.push(node);
  }
  ...
  for (var app in projectData.applications) {
    ...
    for (let missingAppIndex in application.dependenciesToMissingApplications) {
      let missingApp =
        application.dependenciesToMissingApplications[missingAppIndex];
      var edge = {
        color: { color: "#ee0000", highlight: "#ff0000" },
    ...
    for (let unincludedAppIndex in application.dependenciesToUnincludedApplications) {
      let unincludedApp =
        application.dependenciesToUnincludedApplications[unincludedAppIndex];
      var edge = {
        color: { color: "#aaaaaa", highlight: "#eeeeee" },
        smooth: { enabled: false },
        arrows: { to: { enabled: true } },
        from: application.id,
        to: unincludedApp.id,
  ...
  visNetworkHelper.InitNetwork(
    visRenderer.networkInstance,
    function (nodeParams) {
      visRenderer.clickEventListener(nodeParams, projectData);
    }
  );
  visNetworkHelper.ClusterByApplicationGroups(
    visRenderer.networkInstance,
    config["clusterGroups"],
    function (groupName) {
      return visRenderer.clusterNodeRenderer(
        groupName,
        projectData,
        configurationData
      );
...
visRenderer.clickEventListener = function (nodeParams, projectData) {
  ...
  if (app !== false) {
    ...
    let incomingDep = vistectureHelper.GetIncomingDependencies(
      projectData,
      app.id
    );
    ...
    commonTab =
      commonTab +
      `<p class="pt-1"><strong>Consumers:</strong>There are <strong>#${incomingDep.length}</strong> consumers</p>`;
    commonTab =
      commonTab + `<p class="pt-1"><strong>Group:</strong> ${app.group}</p>`;
    commonTab =
      commonTab + `<p class="pt-1"><strong>Team:</strong> ${app.team}</p>`;
    ...
    if (propContent != "") {
      commonTab =
        commonTab +
        `<h6>Properties:</h6> <table class="mt-1 table table-sm small"><tbody>${propContent}</tbody></table> `;
    }
...
visRenderer.getBasicNode = function (application, nodeStyle) {
  ...
  const node = {
    ...
    color: {
      ...
      highlight: {
        ...
        border: colors.highLightBorderColor,
      },
    },
  };
  node.title += " (";
  ...
  if (nodeStyle === "detailed") {
    Object.assign(node, {
      size: 300,
      image: visRenderer.applicationSvgUrl(application, colors),
      shape: "image",
...
visRenderer.getStandardNodeBgColor = function (application) {
  ...
  if (
    application.hasOwnProperty("display") &&
    application.display.hasOwnProperty("color") &&
    application.display.color !== ""
  ) {
...
visRenderer.getColorForGroup = function (groupName) {
  ...
  let colorsScale1 = chroma
    .scale(["#a6f196", "#322b84"])
    .mode("lch")
    .colors(colorScaleSize);
  let colorsScale2 = chroma
    .scale(["#ffd07d", "#923069"])
    .mode("lch")
    .colors(colorScaleSize);
  let colorsScale3 = chroma
    .scale(["#e795ad", "#857dad"])
    .mode("lch")
    .colors(colorScaleSize);
    ...
    for (let groupedDepIndex in application.dependenciesGrouped) {
      ...
      let node = {
        color: {
          ...
          highlight: colors.highLightBorderColor,
        },
        smooth: { enabled: false },
        arrows: { to: { enabled: true } },
        from: groupedDep.sourceApplication.id,
        to: groupedDep.application.id,
        value: groupedDep.dependencies.length,
      };
      if (isBrowserBased) {
        node.dashes = true;
      }
...
visRenderer.applicationSvgUrl = function (application, colors) {
  ...
  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="350px" height="' +
    height +
    'px">' +
    '<rect x="0" y="0" width="100%" height="100%" fill="#efefef" stroke-width="2" stroke="' +
    colors.borderColor +
    '" ></rect>' +
...
visRenderer.clusterNodeRenderer = function (
  groupName,
  projectData,
  configurationData
) {
  ...
  if (configurationData["nodeStyle"] == "detailed") {
    Object.assign(node, {
      size: 300,
      image: visRenderer.clusterNodeSvgUrl(groupName, projectData),
      shape: "image",
      shapeProperties: { useImageSize: true, useBorderWithImage: true },
    });
  }
...
visRenderer.getColorsForApplication = function (application) {
  ...
  if (
    application.hasOwnProperty("display") &&
    application.display.hasOwnProperty("bordercolor") &&
    application.display.bordercolor != ""
  ) {
