...
applicationInit.DrawConfiguredGraph = function () {
  ...
  vistectureHelper.LoadVistectureData(
    selectedSubView,
    networkFilterGroups,
    function (projectData) {
      applicationInit.updateProjectDropdown(
        projectData.availableSubViews,
        config
      );
      applicationInit.updateGroups(
        projectData.applicationsByGroup,
        projectData.availableGroups,
        config
      );
      layout.SetDocumentsMenu(projectData.staticDocumentations);
      visRenderer.RenderNetwork(
...
applicationInit.updateGroups = function (applicationsByGroup, availableGroups) {
  ...
  applicationInit._addSubGroups(
    "#networkClusterGroups",
    applicationsByGroup.subGroups,
  ...
  applicationInit._addSubGroups(
    "#networkFilterGroups",
    availableGroups.subGroups,
    selectedFilterOptions,
    0
  );
};

applicationInit._addSubGroups = function (
  selector,
  subGroups,
  selectedOptions,
  level
  ...
  for (var i in subGroups) {
    ...
    $(selector).append(
      new Option(
        "  > ".repeat(level) + subGroup.groupName,
        subGroup.qualifiedGroupName,
        selected,
        selected
      )
    );
